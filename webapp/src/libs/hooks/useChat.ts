...
import { AuthorRoles, ChatMessageType, Citation } from '../models/ChatMessage';
...
export const useChat = () => {
    ...
    const createChat = async () => {
        const chatTitle = `GAVIN @ ${new Date().toLocaleString()}`;
        try {
            await chatService
    ...
    const importDocument = async (chatId: string, files: File[], global = false) => {
        try {
    ...
    const downloadCitedDocument = async (citation : Citation, openFile : boolean) => {
        try {
            await chatService. downloadCitedDocument(citation, openFile, await AuthHelper.getSKaaSAccessToken(instance, inProgress));
        } catch (e: any) {
...
export function getFriendlyChatName(convo: ChatState): string {
    ...
    // Regex to match the GAVIN timestamp format that is used as the default chat name.
    // The format is: 'GAVIN @ MM/DD/YYYY, hh:mm:ss AM/PM'.
    const autoGeneratedTitleRegex =
        /GAVIN @ [0-9]{1,2}\/[0-9]{1,2}\/[0-9]{1,4}, [0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2} [A,P]M/;
    const firstUserMessage = messages.find(
        (message) => message.authorRole !== AuthorRoles.Bot && message.type === ChatMessageType.Message,
    );

    // If the chat title is the default GAVIN timestamp, use the first user message as the title.

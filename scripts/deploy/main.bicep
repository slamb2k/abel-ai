...
param name string = 'abel'

@description('SKU for the Azure App Service plan')
@allowed([ 'B1', 'S1', 'S2', 'S3', 'P1V3', 'P2V3', 'I1V2', 'I2V2', 'P2MV3' ])
...
@description('Azure OpenAI endpoint to use for embedding. Leaving it empty will cause aiEndpoint to be used instead. (Azure OpenAI only)')
param aiEmbeddingEndpoint string = ''

@secure()
@description('Azure OpenAI or OpenAI API key for embedding. Leaving it empty will cause aiApiKey to be used instead.')
param aiEmbeddingApiKey string = ''
  ...
  properties: {
    ...
    appSettings: [
      ...
      }
      {
        ...
        value: deployNewAzureOpenAI ? openAI.properties.endpoint : (aiEmbeddingEndpoint ?? aiEndpoint)
      }
      {
        name: 'SemanticMemory:Services:AzureOpenAIEmbedding:APIKey'
        value: deployNewAzureOpenAI ? openAI.listKeys().key1 : (aiEmbeddingApiKey ?? aiApiKey)
      }
      {
...
resource swaFrontend 'Microsoft.Web/staticSites@2022-09-01' = {
...
module swaDnsModule 'modules/verifyAzureDns.bicep' = if (webappCustomUrl != '') {
  ...
  scope: resourceGroup('rg-shared')
  params: {
    cnameRecord: webappCustomHost
    dnsZoneName: dnsZoneName
    targetHostname: swaFrontend.properties.defaultHostname
  }
}

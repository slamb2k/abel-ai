...
usage() {
    ...
    echo "  -aieend, --ai-embed-endpoint AI_ENDPOINT                Endpoint for existing Azure OpenAI resource if different for embeddings. If empty, --ai-endpoint will be used instead."
    echo "  -aiekey, --ai-embed-service-key AI_SERVICE_KEY          API key for existing Azure OpenAI resource if different for embeddings. If empty, --ai-service-key will be used instead."
    echo "  -aipm, --ai-planner-model AI_PLANNER_MODEL              OpenAI model to use for planner (default: gpt-35-turbo)"
    echo "  -aiem, --ai-embedding-model AI_EMBEDDING_MODEL          OpenAI model to use for embedding (default: text-embedding-ada-002)"
    echo "  -aicm, --ai-completion-model AI_COMPLETION_MODEL        OpenAI model to use for completions (default: gpt-35-turbo)"
    ...
    echo "  -wappcd, --web-app-custom-domain WEB_APP_CUSTOM_DOMAIN  Optional custom host for the static web app (e.g. XXX.<dns zone>)"
    echo "  -wapicd, --web-api-custom-domain WEB_API_CUSTOM_DOMAIN  Optional custom host for the web api (e.g. XXX.<dns zone>)"
    echo "  -memcd, --memory-custom-domain MEMORY_CUSTOM_DOMAIN     Optional custom host for the memory pipeline (e.g. XXX.<dns zone>)"
...
while [[ $# -gt 0 ]]; do
    ...
    -aiend | --ai-endpoint)
        ...
        shift
        ;;
    -aiekey | --ai-embed-service-key)
        AI_EMBED_SERVICE_KEY="$2"
        shift
        shift
        ;;
    -aieend | --ai-embed-endpoint)
        AI_EMBED_ENDPOINT="$2"
        shift
        shift
        ;;
    ...
    -wr | --web-app-region)
        ...
        shift
        ;;
    -wappcd | --web-app-custom-domain)
        WEB_APP_CUSTOM_DOMAIN="$2"
        shift
        shift
        ;;
    -wapicd | --web-api-custom-domain)
        WEB_API_CUSTOM_DOMAIN="$2"
        shift
        shift
        ;;
    -memcd | --memory-custom-domain)
        MEMORY_CUSTOM_DOMAIN="$2"
        shift
        shift
        ;;
    ...
    -sap | --sql-admin-password)
        ...
        shift
        ;;
    -aipm | --ai-planner-model)
        AI_PLANNER_MODEL="$2"
        shift
        shift
        ;;
    -aiem | --ai-embedding-model)
        AI_EMBEDDING_MODEL="$2"
        shift
        shift
        ;;
    -aicm | --ai-completion-model)
    ...
    cat <<EOF
{
    ...
    "aiEmbeddingEndpoint": { "value": "$([ ! -z "$AI_EMBED_ENDPOINT" ] && echo "$AI_EMBED_ENDPOINT")" },
    "aiEmbeddingApiKey": { "value": "$([ ! -z "$AI_EMBED_SERVICE_KEY" ] && echo "$AI_EMBED_SERVICE_KEY")" },
    "webappCustomHost": { "value": "$([ ! -z "$WEB_APP_CUSTOM_DOMAIN" ] && echo "$WEB_APP_CUSTOM_DOMAIN")" },
    "webapiCustomHost": { "value": "$([ ! -z "$WEB_API_CUSTOM_DOMAIN" ] && echo "$WEB_API_CUSTOM_DOMAIN")" },
    "memoryPipelineCustomHost": { "value": "$([ ! -z "$MEMORY_CUSTOM_DOMAIN" ] && echo "$MEMORY_CUSTOM_DOMAIN")" },
    "dnsZoneName": { "value": "$([ ! -z "$CUSTOM_DOMAIN_ZONE" ] && echo "$CUSTOM_DOMAIN_ZONE")" },    
    "plannerModel": { "value": "$([ ! -z "$AI_PLANNER_MODEL" ] && echo "$AI_PLANNER_MODEL")" },
    "embeddingModel": { "value": "$([ ! -z "$AI_EMBEDDING_MODEL" ] && echo "$AI_EMBEDDING_MODEL")" },
    "completionModel": { "value": "$([ ! -z "$AI_COMPLETION_MODEL" ] && echo "$AI_COMPLETION_MODEL")" }
}
EOF
)

...
{
    public const string DocumentMemoryName = "DocumentMemory";
    ...
    CancellationToken cancellationToken = default)
    {
        ...
        // If the memory name is DocumentMemory, then we need to search for all documents in chat or global documents
        if (!string.IsNullOrWhiteSpace(memoryName) && memoryName == DocumentMemoryName)
        {
